...
TEST_F(MatMulBias, RegisterPromotion) {
  ...
  auto sync1Pos = code.find("__syncthreads()", declPos + 1);
  auto copyToPos =
      code.find("_O_0[0][0] = O[32*b0 + c3][t0 + 32*b1]", sync1Pos + 1);
  auto copyFromPos =
      code.find("O[32*b0 + c3][t0 + 32*b1] = _O_0[0][0]", copyToPos + 1);
  auto sync2Pos = code.find("__syncthreads()", copyFromPos + 1);
  ...
  EXPECT_TRUE(sync1Pos != std::string::npos)
      << "expceted synchronization before copying to register";
  ...
  EXPECT_TRUE(sync2Pos != std::string::npos)
      << "expected synchronization after copying from register";
